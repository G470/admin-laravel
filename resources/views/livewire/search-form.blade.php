        ...
        <div class="row g-3">
            ...
            <div class="col-12 col-md-4">
                <div class="form-floating position-relative">
                    {{-- Suchfeld mit Placeholder und Auto-Complete deaktiviert --}}
            ...
            {{-- STANDORT-FELD MIT AUTO-COMPLETE --}}
            <div class="col-12 col-md-3 d-flex ">
                ...
                {{-- TODO: Standort-Button/Icon für GPS-Funktionalität hinzufügen --}}
            </div>
            ...
            <div class="col-12 col-md-1">
                <div class="position-relative">
                    <div class="country-dropdown" style="height: 58px;">
                        {{-- Dropdown-Button mit ausgewähltem Land --}}
                        <button type="button"
    ...
    $selectedCountry = $this->activeCountries->firstWhere('code', $countryCode);
                                    @endphp
                                    @if($selectedCountry && file_exists(public_path('assets/img/flags/' . strtolower($selectedCountry->code) . '.svg')))
                                        <img src="{{ asset('assets/img/flags/' . strtolower($selectedCountry->code) . '.svg') }}"
                                            alt="{{ $selectedCountry->name }}"
                                            style="width: 20px; height: 15px; margin-right: 8px;">
                                        {{ $selectedCountry->code }}
                                    @elseif($selectedCountry)
                                        <i class="flag-icon flag-icon-{{ strtolower($selectedCountry->code) }} me-2"></i>
                                        {{ $selectedCountry->code }}
                                    @endif
                                @else
                        ...
                        <div class="country-dropdown-menu"
                            ...
                            @foreach($this->activeCountries as $country)
                                <div class="country-option p-2 d-flex align-items-center {{ $countryCode == $country->code ? 'selected' : '' }}"
                                    ...
                                    @if(file_exists(public_path('assets/img/flags/' . strtolower($country->code) . '.svg')))
                                        <img src="{{ asset('assets/img/flags/' . strtolower($country->code) . '.svg') }}"
                                            alt="{{ $country->name }}" style="width: 20px; height: 15px; margin-right: 8px;">
                                    @endif
                    ...
                    {{-- Verstecktes Input für Länderauswahl --}}
                    <input type="hidden" id="countryCode" name="countryCode" value="{{ $countryCode }}">
                </div>
            </div>

            {{-- VERSTECKTE INPUTS FÜR DATUM UND LAND --}}
            ...
            <div class="col-12 col-md-3">
                <div class="form-floating position-relative">
                    ...
                    {{-- Clear-Button für Datumsbereich --}}
                    <button type="button" class="btn btn-lg btn-outline-secondary clear-btn d-none p-4"
